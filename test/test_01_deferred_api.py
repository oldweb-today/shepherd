...
class TestDeferred(object):
    ...
    def test_deferred_start_container(self):
        res = self.client.post('/api/start_deferred/{0}/box-p'.format(self.reqid))
        ...
        res2 = self.client.post('/api/start_deferred/{0}/box-p'.format(self.reqid))
    ...
    def test_deferred_override(self):
        ...
        res = self.client.post('/api/stop_flock/{0}'.format(reqid))
        assert res.json == {'success': True}
